...
constexpr uint32_t CAPTURE_WINDOW_US = 2000; // Thời gian lấy mẫu đỉnh (2ms)
constexpr uint32_t LED_BLINK_MS = 10;        // Thời gian nháy LED báo hiệu
...
// State Machine cho từng Pad
enum PadState { STATE_IDLE, STATE_CAPTURING, STATE_MUTED };
PadState pad_state[PIEZ_CNT];

int kick_val[PIEZ_CNT]; 
uint32_t state_timer[PIEZ_CNT]; // Dùng chung timer cho cả lúc Capture và Mute để tiết kiệm RAM
uint32_t tm_led = 0;
...
void loop() {
    ...
    // 2. State Machine xử lý đa luồng (Multi-pad processing)
    for (byte i = 0; i < PIEZ_CNT; i++) {
        
        // Tối ưu: Chỉ đọc ADC nếu đang IDLE hoặc CAPTURING. Bỏ qua MUTE để tăng tốc vòng lặp.
        int adc = 0;
        ...
        switch (pad_state[i]) {
            ...
            case STATE_CAPTURING:
                ...
                // Hết thời gian lấy mẫu -> Gửi MIDI và chuyển sang Mute
                if (current_micros - state_timer[i] >= CAPTURE_WINDOW_US) {
                    ...
                    if (i == KANAL_HAT && digitalRead(KNOPKA_HAT) == LOW) { 
                        // noteOn(DRUM_CHANEL, kick_note[i] + 2, velocity); // Hi-hat đóng (tuỳ chọn)
                    } else {           
                        noteOn(DRUM_CHANEL, kick_note[i], velocity); 
                    }

                    // Kích hoạt xả áp để triệt tiêu nhiễu
                    pinMode(kick_pin[i], OUTPUT); 
                    digitalWrite(kick_pin[i], LOW);

                    pad_state[i] = STATE_MUTED;
                    state_timer[i] = current_micros; // Reset timer cho quá trình Mute
                }
                break;

            case STATE_MUTED:
                if (current_micros - state_timer[i] >= kick_mute_time[i]) {
                    pinMode(kick_pin[i], INPUT); // Phục hồi chân đọc tín hiệu
                    pad_state[i] = STATE_IDLE;   // Sẵn sàng nhận hit mới
                }
                break;
        }
    }
